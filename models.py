...
from sqlalchemy import Column, String, Integer, Boolean, ForeignKey, CheckConstraint, Text, DateTime, UniqueConstraint,Float
...
class User(Base): # TÀI KHOẢN NGƯỜI DÙNG
    ...
    items = relationship(
        ...
        cascade="all, delete-orphan"  # ✅ Tự động xóa items khi xóa 
    )
...
class PomodoroSettings(Base):
    ...
    selected_track_id = Column(String(255), nullable=True, default=None)
...
class SortSettings(Base):
    __tablename__ = "sort_settings"

    id         = Column(Integer, primary_key=True, autoincrement=True)
    user_id    = Column(Integer, ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True)
    project_id = Column(String(36), ForeignKey("items.id", ondelete="CASCADE"), nullable=False, index=True)
    enabled    = Column(Boolean, nullable=False, default=False)
    # Lưu toàn bộ danh sách rules dưới dạng JSON
    # Ví dụ: [{"field": "priority", "order": "desc", "priority": 1}]
    sort_config = Column(Text, nullable=False, default="[]")

    __table_args__ = (
        # Mỗi user chỉ có 1 bộ sort setting cho mỗi project
        UniqueConstraint("user_id", "project_id", name="uq_sort_user_project"),
    )
...
class FilterSettings(Base):
    ...
    enabled    = Column(Boolean, nullable=False, default=False)
    # Lưu toàn bộ danh sách rules dưới dạng JSON
    # Ví dụ: [{"field": "priority", "operator": "in", "value": ["high", "medium"]}]
    filter_config = Column(Text, nullable=False, default="[]")

    __table_args__ = (
        # Mỗi user chỉ có 1 bộ filter setting cho mỗi project
        UniqueConstraint("user_id", "project_id", name="uq_filter_user_project"),
    )
...
class Roadmap(Base):
    ...
    user_id    = Column(Integer, ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True)
    name       = Column(String(255), nullable=False, default="Roadmap 1")

    # Lưu dạng JSON text — nodes là dict, edges là array
    nodes      = Column(Text, nullable=False, default="{}")
    edges      = Column(Text, nullable=False, default="[]")

    # Canvas state
    n_cnt      = Column(Integer, nullable=False, default=0)   # Bộ đếm node, không reset
    pan_x      = Column(Float, nullable=False, default=0.0)   # Có thể âm
    pan_y      = Column(Float, nullable=False, default=0.0)   # Có thể âm
    zoom       = Column(Float, nullable=False, default=1.0)   # Range: 0.15 → 4.0
...
class ChatMessage(Base):
    ...
    type       = Column(String(50), nullable=True)    # "roadmap" | "folder_tree" | "statistic" | None
    data       = Column(Text, nullable=True)          # JSON string, chỉ có ở assistant
