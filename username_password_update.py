...
# GET /user-id - Chỉ lấy ID của user
@router.get("/user-id", response_model=schemas.UserIdResponse)
def get_user_id(current_user: models.User = Depends(get_current_user)):
    try:
        if not current_user:
        ...
        raise HTTPException(status_code=500, detail=f"Lỗi máy chủ khi lấy user_id: {str(e)}")
